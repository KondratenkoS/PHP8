			...
			<div class="container">
				...
				<div>
					<?php if(empty($_SESSION['auth'])){ ?>
						<a href="index.php" class="btn btn-light" tabindex="-1" role="button" aria-disabled="true">Авторизация</a>
						<a href="registration.php" class="btn btn-light" tabindex="-1" role="button" aria-disabled="true">Регистрация</a>
					<?php } else { ?>
						<a href="logout.php" class="btn btn-light" tabindex="-1" role="button" aria-disabled="true">Выход</a>
					<?php } ?>
				</div>
			</div>
		</nav>
		...
		<div  class="container">
			<table class="table table-bordered">
				...
				<tbody>
					...
					<tr>
						<th><a href="layout_message.php/?topic_name=<?= $topic['topic_name'] ?>&
							user_login=<?= $users[$topic['user_id']]['login'] ?>
							"><?= $topic['topic_name'] ?></a></th>
						...
						<td><?= date('d-m-Y H:i:s', $topic['reply_date']) ?></td>
					</tr>
					<?php } ?>
				</tbody>
			</table>
